...
def generate_collection(tag, dir_name, domains, lang, version, re):
    results = itertools.chain.from_iterable(glob.iglob(os.path.join(root, re))
                        for root, dirs, files in os.walk(os.path.join('conll-2012', version, 'data',
                                                                      tag, 'data', lang)))
    text = ""
    word_count = 0
    sent_count = 0
    ...
    filepath = os.path.join(dir_name, tag + '.csv')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-l', '--language', help='Which language to collect. support english, chinese, arabic', default='chinese')
    parser.add_argument('-d', '--domain', nargs='*', help="What domains to use. If not specified, 'bn mz nw wb' will be used."
    ...
    expected_domains = "bn mz nw wb".split()
    if _domains is None:
